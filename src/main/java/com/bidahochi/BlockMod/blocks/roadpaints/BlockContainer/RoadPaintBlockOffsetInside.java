...
public class RoadPaintBlockOffsetInside extends BaseRoadPaintBlockContainer {
    public RoadPaintBlockOffsetInside(Material mat) {
        super(mat);
    }
    ...
    public void updateTile(World world, int x, int y, int z, EntityLivingBase entity) {
        ...
        for (int i = -1; i < 2; i++) { //x
            for (int j = -1; j < 2; j++) { //y
                for (int k = -1; k < 2; k++) { //z
                    if (world.getBlock(x + i, y + j, z + k) instanceof RoadPaintBlockOffsetInside) {
