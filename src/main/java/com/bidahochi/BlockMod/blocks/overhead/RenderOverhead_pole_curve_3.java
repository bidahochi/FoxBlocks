...
@SideOnly(Side.CLIENT)
    public class RenderOverhead_pole_curve_3 extends TileEntitySpecialRenderer {
        private ResourceLocation texture = new ResourceLocation(FoxBlocks.MODID, "textures/blocks/overheadlines/curve_3x3.png");
        private Modelwpole_3_curve model = new Modelwpole_3_curve();

        @Override
        public void renderTileEntityAt(TileEntity tileEntity, double x, double y, double z, float tick) {
            if(!(tileEntity instanceof TileOverhead_pole_curve_3)){return;}
            GL11.glPushMatrix();
            Tessellator.bindTexture(texture);
            GL11.glTranslated(x + 0.5, y + 0.62, z + 0.5);
            GL11.glRotatef(180F, 1F, 0F, 0F);
            GL11.glRotatef(-90F, 0F, 1F, 0F);
            int dir = ((TileOverhead_pole_curve_3)tileEntity).dir;
            switch(dir) {
                case 0:{// north
