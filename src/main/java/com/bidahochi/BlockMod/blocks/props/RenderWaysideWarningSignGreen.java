...
@SideOnly(Side.CLIENT)
    public class RenderWaysideWarningSignGreen extends TileEntitySpecialRenderer {
        private ResourceLocation texture = new ResourceLocation(FoxBlocks.MODID, "textures/blocks/props/TracksidewarningsignGreen.png");
        private ModelWaysideSign model = new ModelWaysideSign();

        @Override
        public void renderTileEntityAt(TileEntity tileEntity, double x, double y, double z, float tick) {
            if(!(tileEntity instanceof TileWaysideWarningSignGreen)){return;}
            GL11.glPushMatrix();
            Tessellator.bindTexture(texture);
            GL11.glTranslated(x + 0.25, y + 0.62, z + 0.45);
            GL11.glRotatef(180F, 1F, 0F, 0F);
            GL11.glRotatef(90F, 0F, 1F, 0F);
            int dir = ((TileWaysideWarningSignGreen)tileEntity).dir;
