...
public class SittingStairTile extends BaseBlockSittable
{
    ...
    {
        super(material);
        setBlockName("sittingstairtile");
        setHardness(2F);
        setResistance(4.0F);
        setHarvestLevel("pickaxe", 4);
        setStepSound(soundTypeStone);
        setCreativeTab(FoxBlocks.foxBlocksCreativeTabProperProps);
        setSeatPos(new Vec3f[]{new Vec3f(0,-0.5,0)});
    }

    @Override
    public TileEntity createNewTileEntity(World p_149915_1_, int p_149915_2_)
    {
        return new TileSittingStairTile();
    }
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack stack)
    {
        ...
        world.setTileEntity(x,y,z, new TileSittingStairTile(MathHelper.floor_double((entity.rotationYaw / 90.0F) + 2.5D) & 3));
    }
