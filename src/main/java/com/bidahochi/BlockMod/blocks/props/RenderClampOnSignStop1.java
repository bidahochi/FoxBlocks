...
@SideOnly(Side.CLIENT)
    public class RenderClampOnSignStop1 extends TileEntitySpecialRenderer {
        private ResourceLocation texture1 = new ResourceLocation(FoxBlocks.MODID, "textures/blocks/props/ClampOnRailSign_STOP.png");
        private ModelClampOnRailSign model = new ModelClampOnRailSign();

        @Override
        public void renderTileEntityAt(TileEntity tileEntity, double x, double y, double z, float tick) {
            if(!(tileEntity instanceof TileClampOnSignStop1)){return;}
            GL11.glPushMatrix();
            Tessellator.bindTexture(texture1);
            GL11.glTranslated(x + 0.25, y - 0.33, z + 0.45);
            GL11.glRotatef(180F, 1F, 0F, 0F);
            GL11.glRotatef(90F, 0F, 1F, 0F);
            int dir = ((TileClampOnSignStop1)tileEntity).dir;
