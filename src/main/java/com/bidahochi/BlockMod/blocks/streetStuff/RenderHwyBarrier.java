...
@SideOnly(Side.CLIENT)
    public class RenderHwyBarrier extends TileEntitySpecialRenderer {
        private ResourceLocation texture2 = new ResourceLocation(FoxBlocks.MODID, "textures/blocks/streetstuff/hwy_barrier.png");
        private Modelhwy_barrier model = new Modelhwy_barrier();

        @Override
        public void renderTileEntityAt(TileEntity tileEntity, double x, double y, double z, float tick) {
            if(!(tileEntity instanceof TileHwyBarrier)){return;}
            GL11.glPushMatrix();
            Tessellator.bindTexture(texture2);
            GL11.glTranslated(x + 0.5, y + 0.625, z + 0.5);
            GL11.glRotatef(180F, 1F, 0F, 0F);
            GL11.glRotatef(90F, 0F, 1F, 0F);
            int dir = ((TileHwyBarrier)tileEntity).dir;
            switch(dir) {
                case 0:{//west // north
