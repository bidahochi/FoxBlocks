...
@SideOnly(Side.CLIENT)
    public class RenderHwyBarrierDiag extends TileEntitySpecialRenderer {
        private ResourceLocation texture2 = new ResourceLocation(FoxBlocks.MODID, "textures/blocks/streetstuff/hwy_barrier_diag.png");
        private Modelhwy_barrier_diag model = new Modelhwy_barrier_diag();

        @Override
        public void renderTileEntityAt(TileEntity tileEntity, double x, double y, double z, float tick) {
            if(!(tileEntity instanceof TileHwyBarrierDiag)){return;}
            GL11.glPushMatrix();
            Tessellator.bindTexture(texture2);
            GL11.glTranslated(x + 0.5, y + 0.625, z + 0.5);
            GL11.glRotatef(180F, 1F, 0F, 0F);
            GL11.glRotatef(90F, 0F, 1F, 0F);
            int dir = ((TileHwyBarrierDiag)tileEntity).dir;
            switch(dir) {
                case 0:{//west // north
