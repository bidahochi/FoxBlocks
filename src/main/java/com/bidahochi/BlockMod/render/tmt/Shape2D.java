...
public class Shape2D {
	...
	public Shape3D extrude(float x, float y, float z, float rotX, float rotY, float rotZ, float depth, int u, int v, float textureWidth, float textureHeight, int shapeTextureWidth, int shapeTextureHeight, int sideTextureWidth, int sideTextureHeight, float[] faceLengths){
		...
		for(int idx = 0; idx < coords.size(); idx++){
			...
			polySide[0] = new TexturedVertex(verts[idx].vector3F, texU2, texV1);
			polySide[1] = new TexturedVertex(verts[coords.size() + idx].vector3F, texU2, texV2);
			polySide[2] = new TexturedVertex(verts[coords.size() + ((idx + 1) % coords.size())].vector3F, texU1, texV2);
			polySide[3] = new TexturedVertex(verts[(idx + 1) % coords.size()].vector3F, texU1, texV1);
